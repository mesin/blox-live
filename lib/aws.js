...
module.exports = {
  ...
  uninstall: async () => {
    const steps = new Steps(1);
    const step1 = steps.advance('Uninstall in progress, please wait...', 'desktop_computer').start();
    const otp = conf.get('otp');
    if (!otp) throw new Error('otp not found');
    const instanceId = conf.get('instanceId');
    if (!instanceId) throw new Error('instanceId not found');
    const storedCredentials = conf.get('credentials');
    if (!storedCredentials) throw new Error('credentials settings not found');
    awsOptions.credentials = { ... storedCredentials };
    ec2 = new AWS.EC2(awsOptions);
    await ec2.terminateInstances({ InstanceIds: [instanceId] }).promise();
    await ec2.waitFor('instanceTerminated', { InstanceIds: [instanceId] }, () => {}).promise();
    const securityGroupId = conf.get('securityGroupId');
    if (!securityGroupId) throw new Error('securityGroupId not found');
    await ec2.deleteSecurityGroup({
      GroupId: securityGroupId,
      DryRun: false
    }).promise();
    const addressId = conf.get('addressId');
    if (!addressId) throw new Error('addressId not found');
    const publicIp = conf.get('publicIp');
    if (!publicIp) throw new Error('publicIp not found');
    await ec2.releaseAddress({
      AllocationId: addressId
    }).promise();
    const { pairId } = conf.get('keyPair');
    if (!pairId) throw new Error('pairId not found');
    await ec2.deleteKeyPair({
      KeyPairId: pairId
    }).promise();
    step1.success('Successfully uninstalled', 'white_check_mark')
  },

...
module.exports = {
  ...
  setup: async () => {
    ...
    // status.start();
    try {
      const step2 = steps.advance('Creating ec2 key pair...', 'key', 'EC2.createKeyPair').start();
      // 1. get stored pair key or create new one
      const storedKeyPair = conf.get('keyPair');
      if (!storedKeyPair) {
        const keyPairData = await ec2.createKeyPair({ KeyName: keyName }).promise();
        conf.set('keyPair', {
          pairId: keyPairData.KeyPairId,
          privateKey: keyPairData.KeyMaterial
        });  
      }
      step2.success(`Successfully created key pair`, 'white_check_mark');
      const step3 = steps.advance('Allocating ip address...', 'round_pushpin', 'EC2.allocateAddress').start();
      ...
      step3.success(`Successfully setup ip address`, 'white_check_mark');
      const step4 = steps.advance('Setting up Ubuntu t2.micro instance...', 'desktop_computer', 'EC2.runInstances').start();
      ...
      if (!instanceId) {
        ...
        const data = await ec2.runInstances({
          ImageId: 'ami-00e3060e4cb84a493', // ubuntu 16.04LTS for us-west-1
      ...
      step4.success(`Successfully setup server on ip ${publicIp}`, 'white_check_mark');
    } catch (e) {
      throw new Error(e);
    } finally {
      // status.stop();

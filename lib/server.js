  ...
  setupEnv: async () => {
    ...
    if (!dockerImageIsUp) {
      ...
      */
      ...
      const vaultVersion = `0.0.8`;
      currentStep = steps.advance(`Run dockerized vault plugin v${vaultVersion} container...`, 'hammer_and_wrench', 'docker run').start();
      output = await ssh.execCommand(`curl -L "https://raw.githubusercontent.com/bloxapp/vault-plugin-secrets-eth2.0/v${vaultVersion}/docker-compose.yml" -o docker-compose.yml && docker-compose up -d vault-image`, {});
      await delay(30000);
      currentStep.success(`Vault plugin v${vaultVersion} is up', 'white_check_mark`);
   }

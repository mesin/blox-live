...
module.exports = {
  setupEnv: async () => {
    ...
    const steps = new Steps(6);
    let currentStep;
    currentStep = steps.advance('Connecting to the server thru ssh...', 'link', 'ssh.connect').start();
    ...
    currentStep.success(`Successfully connected`, 'white_check_mark');
    ...
    if (!dockerImageBuiltAready) {
      ...
      currentStep = steps.advance('Clone git repo...', 'hammer_and_wrench', 'yum install git').start();

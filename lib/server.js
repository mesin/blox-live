...
module.exports = {
  setupEnv: async () => {
    ...
    if (true) {
      output = await ssh.execCommand('cd vault-plugin && sudo cat data/keys/vault.root.token', {});
      rootToken = output.stdout;  
      if (!rootToken) throw new Error('root vault-plugin key not found');
      conf.set('vaultRootToken', rootToken);
    }
    console.log(`curl --request POST --data '{"otp": "${otp}", "url": "http://${publicIp}:8200", "accessToken": "${rootToken}"}'  http://api.stage.bloxstaking.com/wallets/root`)

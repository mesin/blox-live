  ...
  program
    ...
    .action(async(options) => {
      ...
      try {
        ...
        console.log(chalk.blue(`> Key vault ${confClient.get('keyVaultVersion')} was installed on server http://${confClient.get('publicIp')}:8200.`));
      } catch(err) {
  ...
  program
    ...
    .action(async() => {
      ...
      try {
        ...
        console.log(chalk.blue(`> New key vault ${confClient.get('keyVaultVersion')} was installed on server url: http://${confClient.get('publicIp')}:8200.`));
      } catch(err) {
        console.log(chalk.red(err.message));
      }
      process.exit();

...
function* startLoadingMnemonic() {
  try {
    const mnemonicPhrase = yield call([seedService, seedService.mnemonicGenerate]);
...
function* startSavingMnemonic(action) {
  ...
  try {
    yield call([seedService, seedService.storeMnemonic], mnemonic, password);
...
function* startLoadingLatestVersion() {
  try {
    const latestVersion = yield call([walletService, walletService.getLatestTag]);

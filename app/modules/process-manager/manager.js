...
const run = async () => {
  clear();
  console.log(
    chalk.yellow(figlet.textSync('Blox Staking', { horizontalLayout: 'full' }))
  );
  ...
  program.version(version).name('blox-live');
  ...
  program
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.log(
          chalk.blue(
            `> Key vault ${confClient.get(
              'keyVaultVersion'
            )} was installed on server http://${confClient.get(
              'publicIp'
            )}:8200.`
          )
        );
  ...
  program
    ...
    .action(async () => {
      if (!conf.get('otp'))
        throw new Error('installation results was not found');
      const storeName = `blox-${conf.get('otp')}`;
      const confClient = new Configstore(storeName);
      const aws = new AWSLib(storeName);
      const keyVault = new KeyVaultLib(storeName);
  ...
  program
    ...
    .action(async () => {
      ...
      try {
        ...
        !confClientTmp.get('reinstall.keyVault.done') &&
          (await keyVault.reinstall());

        console.log(chalk.blue('- Truncate old server'));
        !confClient.get('reinstall.awsOld.done') &&
          (await awsOld.uninstallOldServer());
        ...
        console.log(
          chalk.blue(
            `> New key vault ${confClient.get(
              'keyVaultVersion'
            )} was installed on server url: http://${confClient.get(
              'publicIp'
            )}:8200.`
          )
        );
